...
<?php
	...
	echo $query."<br><br>";
	//creando tabla
	get_assist_data(str_replace("<br>", " ", $query), $table_title);

	echo "<input type=\"button\" onclick=\"tableToExcel('asistencia')\" value=\"Export to Excel\">";
	//echo "<input type=\"button\" onclick=\"exportTableToExcel('tblassistants', filename = 'asistencia')\" value=\"Export to Excel\">";
	
	//ejecutando query y creando tabla
	function get_assist_data($consulta, $table_title){
		...
		if($result->num_rows > 0){
			...
			echo '<tr><td colspan="'.$fieldcount.'"><b style="font-size: 20px;">Lista de asistencia a evento: '.get_event_name($_POST["evento"]).'</b><br>
														<b>Fecha del evento: '.get_evento_date($_POST["evento"]).'</b><br>
														<b>'.$table_title.'</b><br><br></td></tr>';
